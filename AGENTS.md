...
- Hidden model markers must stay out of speech, captions, and conversational history.
  Chatterbox sound tags (`[laugh]`, see `ts-backend/src/speechTags.ts`) reach only a
  Chatterbox voice; captions, history, and other engines get the stripped text. Style
  tags such as `[whispering]` are inert on Turbo (measured) and are never suggested.
